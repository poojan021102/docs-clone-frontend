    ...
    useEffect(() => {
        const temp = async() => {
            ...
            if(appContext.user?.userId && cookie){
                const response = await sharedDocument(appContext.user.userId, appContext.user.email, cookie);
                if(response.status){
        ...
        return (
            ...
            <div className="mt-2 w-[40vw] h-[100%] bg-slate-100 flex justify-center items-start">
                <div className="w-[100%] flex flex-col justify-center items-center">
                    ...
                    <Link to = "/allDocuments" className="p-2 rounded-md text-xs md:text-md m-4 pl-3 pr-3 text-slate-200 font-semibold bg-blue-500 hover:bg-blue-600">My Documents</Link>
                        ...
                        <tbody>

                            {
                                allDocuments && allDocuments.map((document) => {
                                    return(
                                        <tr className="pt-3 pb-3 bg-white shadow-xl font-bold" key = {document.documentId}>
                                            <td className="pt-2 border-y-[2px]text-center text-xs md:text-md underline text-blue-400"><div className="flex justify-around items-center"><img className="w-6 h-6" src={docsLogo} alt="Document" /><Link to = {`/document/${document.documentId}`}>{document.documentTitle}</Link></div></td>
                                        </tr>
                                    )
                                })
                            }
