...
export default function DocumentPage(){
            ...
            const temp = async() => {
                ...
                if(appContext.user && appContext.user.userId){
                    ...
                    if(documentId && authToken){
                        const checkAccess = await canAccess(appContext.user.userId, appContext.user.email, documentId, authToken);
                        if(!checkAccess.status){
                           ...
                           if(authToken && documentId){
                                const document = await getDocumentContent(appContext.user?.userId, appContext.user?.email, authToken, documentId);
    ...
    return(
        <div>
            {
                appContext.loggedInStatus === "inProgress" || appContext.loggedInStatus === "logout" || !documentId || !appContext || !appContext.user || loading ? <Loading message={loadingMessage}/>:<ShowEditor documentTitle = {documentTitle.current} documentContent = {documentContent.current} appContext = {appContext} documentId={documentId}/>
            }           
        </div>
    )
}